...
def test_pe_files_in_deployment_zip():
    ...
    with zipfile.ZipFile("deploy_windows_x86_64.zip") as zip_file:
...
def test_python_path(name):
    ...
    assert "environment" in deployment_environment, f"{deployment_environment = }"
    assert "PYTHONPATH" in deployment_environment["environment"], f"{deployment_environment = }"

    python_paths = deployment_environment["environment"]["PYTHONPATH"].split(os.pathsep)
    assert python_paths

    for path in python_paths:
        assert any(name for name in zipped_names if path in name), f"{path = } not found in {python_paths = }"

    assert "__main__.py" in zipped_names, f"{zipped_names = }"


@pytest.mark.parametrize("zip_name", ["default.zip", f"deploy_{sys.platform}_{platform.machine()}.zip"])
def test_python_zip(zip_name):
    zip_path = pathlib.Path(zip_name)
    if zip_path.exists():
        args = [sys.executable, os.fspath(zip_path.resolve())]
        result = subprocess.run(args, stdout=subprocess.PIPE)
        assert result.returncode == 0, f"{args = } failed at {os.getcwd() = }"

        stdout = result.stdout.decode()
        assert "module 'numpy'" in stdout, f"{stdout = }"
        assert "/bin/python3" in stdout or "python.exe" in stdout, f"{stdout = }"
