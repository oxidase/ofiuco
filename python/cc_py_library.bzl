def _cc_py_library_impl(ctx):
    ...
    linking_outputs = cc_common.link(
        ...
        name = ctx.label.name + ".todo", #artifact_name_patterns
        language = "c++",
    ...
    # Collect runfiles
    runfiles = ctx.runfiles(transitive_files = depset([linking_outputs.executable] + solibs + ctx.files.data))
    for dep in ctx.attr.deps:
        runfiles = runfiles.merge(dep[DefaultInfo].data_runfiles)

    for data in ctx.attr.data + [dep for dep in deps if PyInfo in dep]:
...
cc_py_library = rule(
    ...
    attrs = {
        ...
        "_allowlist_function_transition": attr.label(default = "//tools/whitelists/function_transition_whitelist"),
    },
    fragments = ["cpp", "py"],
    toolchains = [
        "@bazel_tools//tools/cpp:toolchain_type",
